...
export function playStrikeSound(strikeIndex: number): void {
  ...
  if (!ac) return;
  if (ac.state !== "running") {
    // The browser requires a user gesture before the audio context
    // can be resumed. Do not attempt to resume here because this
    // function may be called from the startup animation.
    //
    // The first user gesture is handled by initGavelSound().
    return;
  }
