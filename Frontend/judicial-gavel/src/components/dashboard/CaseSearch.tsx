    ...
    <div>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search matters, case numbers, parties, FIRs, police stations or CNRs…"
          autoComplete="off"
          spellCheck={false}
          className={`focus-legal h-16 w-full border border-border bg-surface/45 py-4 pl-13 pr-24 font-display text-[15px] tracking-wide text-parchment outline-none transition-all duration-300 placeholder:font-sans placeholder:text-sm placeholder:font-light placeholder:text-muted-foreground/60 hover:border-brass-dim focus:border-brass focus:bg-surface/70 ${
            value
              ? "pr-24"
              : "pr-14"
          }`}
        ...
        {value && (
          <button
            ...
            className="focus-legal absolute right-14 top-1/2 flex -translate-y-1/2 items-center gap-1 px-1 py-1 text-[10px] tracking-[0.12em] text-muted-foreground uppercase transition-colors hover:text-brass"
          >
            Clear
            <X className="h-3 w-3" />
          </button>
            ...
            <div className="mt-4 grid grid-cols-1 gap-x-5 gap-y-4 sm:grid-cols-2 lg:grid-cols-4">
              {FILTER_FIELDS.map(
                (field) => (
                  ...
                  >
                    <label
                      htmlFor={`case-filter-${field.key}`}
                      className="label-legal"
                    >
                      {
                        field.label
                      }
